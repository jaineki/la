...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  if (!userQuery.trim()) {
    res.status(400).json({ status: false, error: 'Missing query parameter.' });
    return;
  }
  ...
  try {
    ...
    if (!upstream.ok) throw new Error(`External API status ${upstream.status}`);

    const data = await upstream.json() as { response?: string; reply?: string; operator?: string };
